...
summary_cv_data <- function(cv_dataset)
{
                                   ...
                                   h)
  {
    ...
    errors_median <-
      apply(errors, 2, median) # robust to outliers, and more representative of a typical value
  ...
  colnames(results_mean) <-
    colnames(results_sd) <- paste0("h", horizons)
...
cv_data_ranking <- function(cv_results)
{
  ...
  ranks <- foreach::foreach(i = 1:n_datasets, .combine = rbind) %do% {
    cbind.data.frame(datasets_names[i],
                     fcast_methods,
  ...
  mean_ranks <- as.data.frame(dplyr::summarise(
    by_method,
